...
public class DataSourceServiceImpl implements DataSourceService {
    ...
    private void validateFields(DataSourceConfiguration entity, List<String> errors) {
        ...
        if (value != null) {
            ...
            switch (entity.getFetchMode()) {
                case SYMLINK:
                case COPY:
                case CHECK:
                    break;
                default:
                    errors.add(String.format("[fetchMode] when 'localArchivePath' is set, this has to be one of %s, %s or %s",
                                             FetchMode.COPY.value(), FetchMode.SYMLINK.value(), FetchMode.CHECK));
            }
        }
