    ...
    public boolean downloadStarted(EOProduct product) {
        synchronized (this.lock) {
            try {
                ...
                logger.warning(String.format("Exception in downloadStarted: %s", ExceptionUtils.getStackTrace(logger, ex)));
                return false;
            }
        }
    }
            ...
            try {
                ...
                if (dbProduct != null) {
                            ...
                            if (this.checkMd5 && Satellite.Sentinel1.name().equals(product.getProductType())) {
                                ...
                                try {
                                    ...
                                    logger.severe(String.format("Cannot open product %s metadata. Reason: %s",
                                            product.getName(), ExceptionUtils.getStackTrace(logger, e)));
                                }
                            }
                    ...
                    if (Constants.S2L2A_PRODUCT_TYPE.equals(product.getAttributeValue(CommonParameterNames.PRODUCT_TYPE))) {
                        ...
                        final String fullPath = dbProduct.getFullPath();
                        l2aProduct.setFullPath(fullPath);
                        ...
                        try {
                            Sentinel2L2AQuicklookGenerator qlGenerator = new Sentinel2L2AQuicklookGenerator();
                            if (fullPath == null) {
                                throw new Exception("[null product path]");
                            }
                            Path path = qlGenerator.handle(Paths.get(fullPath));
                            if (path == null) {
                                throw new Exception("[null]");
                            }
                            quickLookPath = path.toString();
                        } catch (Exception e) {
                            logger.warning(String.format("Cannot create quicklook for product %s. Reason: %s",
                                    dbProduct.getProductName(), e.getMessage()));
                            quickLookPath = product.getQuicklookLocation();
                        }
                        l2aProduct.setQuickLookPath(quickLookPath);
                ...
                logger.warning(String.format("Exception in downloadCompleted: %s", ExceptionUtils.getStackTrace(logger, ex)));
            }
        }
    }
    ...
    private void handleDownloadUnsuccessful(EOProduct product, String reason, Status status) {
        synchronized (this.lock) {
            try {
                ...
                // The "retry" attribute is set only if there is a secondary datasource defined.
                // In this case, we should keep the DOWNLOADING status for the 2nd datasource attempt
                if (status == Status.FAILED && product.getAttributeValue("retry") == null) {
                ...
                logger.warning(String.format("Exception in handleDownloadUnsuccessful: %s", ExceptionUtils.getStackTrace(logger, ex)));
            }
        }
    }
