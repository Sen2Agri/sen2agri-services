...
public class ProductRepository extends NonMappedRepository<HighLevelProduct> {
    ...
    List<HighLevelProduct> findProductByName(String name) {
        return new HighLevelProductTemplate() {
            @Override
            protected String conditionsSQL() { return "WHERE name = ?"; }
    ...
    List<HighLevelProduct> findByDownloadedProduct(int downloadHistoryId, int productTypeId) {
        return new HighLevelProductTemplate() {
            @Override
            protected String conditionsSQL() { return "WHERE downloader_history_id = ? and product_type_id = ?"; }
    ...
    private List<HighLevelProductCount> getProductCountBySite(int...siteIds) {
        ...
        final Map<Integer, ProductTypeInfo> types = persistenceManager.getProductTypes().stream().collect(Collectors.toMap(ProductTypeInfo::getId, Function.identity()));
        jdbcTemplate.query("SELECT site_id, product_type_id, COUNT(id) FROM public.product WHERE site_id IN (" +
                                    ...
                                    resultSet -> {
                                        if (resultSet.isFirst()) {
                                            do {
                                                ...
                                                row.setProductType(types.get((int) resultSet.getShort(2)));
        ...
        if (sites != null && sites.size() > 0) {
            ...
            for (Site s : sites) {
                ...
                for (ProductTypeInfo productTypeInfo : productTypes) {
                    typeCount = countList.stream().filter(c -> c.getSiteId() == s.getId() && c.getProductType().getId() == productTypeInfo.getId()).findFirst().orElse(null);
    ...
    HighLevelProduct saveProduct(HighLevelProduct product) {
        ...
        params.put("_job_id", product.getJobId());
    ...
    HighLevelProduct setArchived(HighLevelProduct product) {
        ...
        jdbcTemplate.update(connection -> {
            PreparedStatement statement =
                    connection.prepareStatement("UPDATE public.product " +
    ...
    protected String selectQuery() {
        return "SELECT id, site_id, processor_id, name, full_path, " +
                ...
                "satellite_id, product_type_id, geog, tiles, orbit_type_id, quicklook_image, downloader_history_id, job_id FROM public.product ";
    }
    ...
    private abstract class HighLevelProductTemplate extends Template {
        ...
        protected RowMapper<HighLevelProduct> rowMapper() {
            return (resultSet, i) -> {
                ...
                value = resultSet.getInt(17);
                if (value > 0) {
                    product.setJobId(value);
                }
                return product;
            };
        }
