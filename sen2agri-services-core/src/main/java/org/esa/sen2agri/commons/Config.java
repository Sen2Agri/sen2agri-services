...
public class Config {
    ...
    public static DataSourceConfiguration refreshDbConfiguration(DataSourceConfiguration config) {
        final DataSourceConfiguration dsc = persistenceManager.getDataSourceConfiguration(config.getId());
        final Properties properties = new Properties();
        final DataSource<?, ?> dataSource = DataSourceManager.getInstance().get(dsc.getSatellite().name(), dsc.getDataSourceName());
        final String dsKey = dataSource.getClass().getSimpleName() + "." + dsc.getSatellite().name();
        configuration.stringPropertyNames()
                .stream()
                .filter(n -> n.startsWith(dsKey))
                .forEach(n -> properties.put(n.replace(dsKey + ".", ""), configuration.get(n)));
        dsc.setAdditionalSettings(properties);
        final int index = dataSourceConfigurations.indexOf(config);
        if (index != -1) {
            dataSourceConfigurations.set(index, dsc);
        } else {
            dataSourceConfigurations.add(dsc);
        }
        final ThreadPoolExecutor executor = dataSourceExecutors.remove(config);
        if (executor != null) {
            dataSourceExecutors.put(dsc, executor);
        }
        return dsc;
    }
    ...
    public static void reload() {
        ...
        final long timeout = Long.parseLong(Config.getSetting(ConfigurationKeys.DOWNLOADER_ACTION_TIMEOUT, "9000"));
        ...
        for (DataSourceConfiguration ds : dataSourceConfigurations) {
            ...
            if (dataSourceExecutors.containsKey(ds)) {
                ...
                if (executor != null && !executor.isShutdown() && !executor.isTerminated()) {
                    if (executor.getMaximumPoolSize() != maxConnections) {
                        ...
                        backgroundWorker.submit(() -> {
                            ...
                            final NamedThreadPoolExecutor poolExecutor =
                                    new NamedThreadPoolExecutor(ds.getDataSourceName() + "-" + ds.getSatellite().friendlyName(),
                                    maxConnections);
                            poolExecutor.setTimeout(timeout, TimeUnit.SECONDS);
                            dataSourceExecutors.put(ds, poolExecutor);
                        });
                    }
                }
            } else {
                ...
                poolExecutor.setTimeout(timeout, TimeUnit.SECONDS);
                dataSourceExecutors.put(ds, poolExecutor);
                logger.fine(String.format("Initialized datasource [%s,%s] with %d max connections and timeout %ds",
                                          ds.getDataSourceName(), ds.getSatellite().friendlyName(), maxConnections, timeout));
            }
        }
    ...
    public static ThreadPoolExecutor getWorkerFor(DataSourceConfiguration dataSourceConfiguration) {
        ...
        final ThreadPoolExecutor poolExecutor = dataSourceExecutors.get(dataSourceConfiguration);
    ...
    public static DataSourceConfiguration getDownloadConfiguration(short id) {
        return dataSourceConfigurations.stream().filter(c -> c.isEnabled() && (c.getScope() & Scope.DOWNLOAD) != 0 && c.getId() == id)
                .findFirst().orElse(null);
    }
