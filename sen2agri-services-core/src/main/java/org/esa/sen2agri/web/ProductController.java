...
public class ProductController extends ControllerBase {
    ...
    public ResponseEntity<?> getProducts(@RequestParam("userId") int userId,
                                         ...
                                         @RequestParam("productTypeId") short productTypeId,
                ...
                for (ProductFileInfo info : infos) {
                    if (info.getPath() != null) {
                        ...
                        try {
                            ...
                            final Logger logger = Logger.getLogger(ProductController.class.getName());
                            logger.warning(String.format("Cannot determine size of path %s. Reason: %s",
                                                         path, ExceptionUtils.getStackTrace(logger, e)));
                        }
                    }
                }
            }
    ...
    public ResponseEntity<?> upload(@RequestParam("siteId") int siteId,         // the site
        ...
        try {
            ...
            FileUtilities.ensureExists(localFolder);
    ...
    public ResponseEntity<?> importProducts(@RequestParam("siteId") short siteId,
        ...
        if (sourceDir == null || siteId <= 0 || satelliteId <= 0) {
            return prepareResult("Invalid argument.", ResponseStatus.FAILED);
        } else {
            ...
            if (Config.getDownloadConfiguration(site, satellite) == null) {
                return prepareResult("No datasource with scope DOWNLOAD is configured, cannot determine the target location",
                                     ResponseStatus.FAILED);
            }
            asyncExecute(() -> { importProductsDelegate(sourceDir, site, satellite, link); });
            return prepareResult("Import request submitted. Please check logs for operation progress.",
    ...
    private void importProductsDelegate(String sourceDir, Site site, Satellite satellite, boolean link) {
        ...
        if (Files.exists(sourcePath)) {
            ...
            try {
                ...
                List<Path> folders = FileUtilities.listFolders(sourcePath, 2);
                int count = 0;
                ...
                final Geometry siteExtent = adapter.marshal(site.getExtent());
                    ...
                    try {
                        //if (Files.isDirectory(folder) && !folder.equals(sourcePath)) {
                            ...
                            if (!folder.toString().startsWith(downloadPath.toString())) {
                                ...
                                if (!Files.exists(targetPath)) {
                                    if (link) {
                                        ...
                                        FileUtilities.linkFile(folder, targetPath);
                                    } else {
                            ...
                            if (metadata != null) {
                                ...
                                if (!(siteExtent.intersects(adapter.marshal(product.getGeometry())))) {
                                    warn("Product %s does not cover the extent of the site %s", product.getName(), site.getShortName());
